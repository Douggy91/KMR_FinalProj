...
# conn = pymysql.connect(host="ec2-54-180-32-135.ap-northeast-2.compute.amazonaws.com",user="user",password="1234", db='foodmenu')
curs = conn.cursor()
...
class Shop:
	...
	def interactiveMenu(self):
		...
		self.quantity = {}
		r = 1
		for food in (sorted(self.menu.menu.values(), key=operator.attrgetter("ranking"),reverse=True)):
	...
	def submit_order(self):
		...
		for foodname in self.order.keys():
			...
			sqlQuery = "UPDATE "+self.name+"_menu SET ranking="+repr(food.ranking+qnty)+" WHERE food="+repr(food.name)+";"
			curs.execute(sqlQuery)
			conn.commit()
			sqlQuery = "INSERT INTO "+self.name+"_order_b VALUES ("+repr(timedate)+", "+repr(food.name)+","+repr(qnty)+");"
			curs.execute(sqlQuery)
			conn.commit()
			sqlQuery = "DELETE FROM "+self.name+"_order_b WHERE count = 0;"
			curs.execute(sqlQuery)
			conn.commit()	
			sqlQuery = "INSERT INTO "+self.name+"_order VALUES ("+repr(timedate)+", "+repr(food.name)+","+repr(qnty)+",(SELECT COUNT(DISTINCT date_)+1 ord FROM "+self.name+"_order_b));"
			curs.execute(sqlQuery)
			conn.commit()
			sqlQuery = "DELETE FROM "+self.name+"_order WHERE count = 0;"
			curs.execute(sqlQuery)
			conn.commit()	
		...
		self.Return_button = Button(self.shopFrame, text="Return_button", command=lambda:self.proceed_str(shopname))
		self.Return_button.config(bg="green3")
	...
	def click_add(self):
		...
		ranking = 0
		food = foodmenu.Food(name,price,ranking)
		self.menu.add(food)
		self.displayMenu()
	...
	def savemenu(self):
		...
		for food in self.menu.menu.values():
			sqlQuery = "INSERT INTO "+self.name+"_menu VALUES("+repr(food.name)+", "+repr(food.price)+", "+repr(food.ranking)+");" 
			curs.execute(sqlQuery)
		conn.commit()
		print("Menu Updated")	
