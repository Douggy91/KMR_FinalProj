...
# conn = pymysql.connect(host="ec2-54-180-32-135.ap-northeast-2.compute.amazonaws.com",port=3306, user='user',password='1234',db='foodmenu', charset='utf8')
curs = conn.cursor()
try:
...
class LoginPage():
	...
	def reg(self):
		...
		curs.execute(sqlQuery)
		conn.commit()
		sqlQuery = "CREATE TABLE "+shopname+"_order(date_ datetime, food varchar(20), count int, order_no int);"
		curs.execute(sqlQuery)
		conn.commit()
		sqlQuery = "CREATE TABLE "+shopname+"_order_B(date_ datetime, food varchar(20), count int);"
		curs.execute(sqlQuery)
		conn.commit()
